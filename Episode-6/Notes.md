...
Meaning, we are dividing software into small, well-defined modules enables teams to use functions for multiple purposes.
...
Meaning, we are not dividing software into small, well-defined modules, we use every service like, database, server or a UI of the application, in one Application file.
...
- ![Monilith VS Microservice](image.png)

### Why do we need a `useEffect` Hook?

`useEffect Hook` is a javascript function provided by `react`. The useEffect Hook allows you to `eliminate side effects` in your components. Some examples of side effects are: `fetching API data`, `directly updating the DOM`, and `setting up subscriptions or timers`, etc that can lead to unwarranted side-effects.
...
If we pass any variable(suppose currentState) inside the `[]`, it triggers the callback function and changes the state of the application.
...
A: `Async`: It simply allows us to write promise-based code as if it was synchronous and it checks that we are not breaking the execution thread. It operates asynchronously via the event loop. Async functions will always return a promise. It makes sure that a promise is returned and if it is not returned then JavaScript automatically wraps it in a promise which is resolved with its value.
...
x, y => parameters (hold the values)
2, 3 => arguments (are the values)

### `const [status, setStatus] = useState('')` => How is `status` updated if it is declared with `const`?

`NOTE: Whenever there is a state change, React re-renders the entire component`.
Keeping the above not in mind, whenever the component is re-rendered, a `const` `status` variable is declared, only this time, the default value is the updated value. 
